// Renders the AiDecisionPanel client interface for training execution workflows.
...
export function AiDecisionPanel({ batch }) {
  ...
  const canRegenerate = Boolean(data?.cached || data?.generatedBy === 'openai' || data?.aiFallbackReason)

  const generate = async () => {
    const now = Date.now()
    if (generating || now - lastGenerateAt.current < 1000) return
    lastGenerateAt.current = now
    setGenerating(true)
    setMessage('')
    try {
      const result = await generateAiInsights(batch.batchId, canRegenerate)
      setData(result)
      setMessage(
        result.aiFallbackReason === 'rate_limited'
          ? 'AI provider is temporarily rate-limited. Showing rule-based fallback.'
          : result.cached
            ? 'Showing cached AI insights because batch evidence has not changed.'
            : 'AI insights generated from current batch evidence.',
      )
    } catch (error) {
      setMessage(error.message || 'Unable to generate AI decision insights.')
    } finally {
      setGenerating(false)
    }
  }
  ...
  useEffect(() => {
    let active = true
    getCachedAiInsights(batch.batchId)
      .then((result) => {
        if (active) {
          setData(result)
          if (result.aiFallbackReason === 'rate_limited') {
            setMessage('AI provider is temporarily rate-limited. Showing rule-based fallback.')
          }
        }
      })
      .catch((error) => {
    ...
    <section className="rounded-xl border border-cyan-300/15 bg-cyan-300/[0.035] p-4 sm:p-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        >
          ...
          {canRegenerate ? 'Regenerate AI Insights' : 'Generate AI Insights'}
        </button>
      </div>

      {message ? <p className="mt-3 text-xs text-cyan-100">{message}</p> : null}
      {loading && !data ? <p className="mt-4 text-sm text-zinc-400">Loading cached decision insights...</p> : null}
      {!loading && data && !data.cached && data.generatedBy === 'rules' ? (
        <p className="mt-3 text-xs text-zinc-400">Rule-based insights are shown. Select Generate AI Insights to request an AI-refined analysis.</p>
      ) : null}
